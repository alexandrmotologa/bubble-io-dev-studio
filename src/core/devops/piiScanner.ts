...
export class PiiScanner {
  ...
  /**
   * Generates a complete Markdown Audit Report suitable for GDPR, compliance, and client delivery
   */
  public static generateMarkdownReport(report: PiiAuditReport): string {
    let md = `# 🛡️ Bubble.io PII & Privacy Security Audit Report\n\n`;
    md += `**Application:** \`${report.appName}\`  \n`;
    md += `**Environment:** \`${report.environment}\`  \n`;
    md += `**Audit Timestamp:** \`${report.scannedAt}\`  \n`;
    md += `**Audited Tables:** ${report.totalTypes} Data Types (${report.totalFields} Total Fields)  \n\n`;

    md += `## 📊 Executive Risk Summary\n\n`;
    md += `| Risk Level | Finding Count | Action Required |\n`;
    md += `| :--- | :---: | :--- |\n`;
    md += `| 🔴 **CRITICAL RISK** | **${report.criticalCount}** | Immediate remediation in Bubble Privacy Rules |\n`;
    md += `| 🟠 **HIGH RISK** | **${report.highCount}** | Restrict field visibility to authorized roles |\n`;
    md += `| 🔵 **MEDIUM RISK** | **${report.mediumCount}** | Review public Data API exposure |\n\n`;

    md += `## 📋 Detailed Vulnerability Findings (${report.findings.length})\n\n`;
    md += `| Table | Field | Severity | Category | Remediation |\n`;
    md += `| :--- | :--- | :---: | :---: | :--- |\n`;

    for (const f of report.findings) {
      const sevEmoji = f.severity === 'CRITICAL' ? '🔴 CRITICAL' : f.severity === 'HIGH' ? '🟠 HIGH' : '🔵 MEDIUM';
      md += `| \`${f.table}\` | \`${f.field}\` | ${sevEmoji} | \`${f.category}\` | ${f.recommendation} |\n`;
    }

    md += `\n---\n*Report generated by Bubble.io Dev Studio v1.4.0*\n`;
    return md;
  }

  /**
   * Generates a ready-to-paste Bubble Privacy Rules configuration guide
   */
  public static generatePrivacyRulesMatrix(report: PiiAuditReport): string {
    let txt = `=================================================================\n`;
    txt += `BUBBLE.IO PRIVACY RULES REMEDIATION MATRIX\n`;
    txt += `Application: ${report.appName} (${report.environment})\n`;
    txt += `Total Protected Fields: ${report.findings.length}\n`;
    txt += `=================================================================\n\n`;

    const byTable: Record<string, typeof report.findings> = {};
    for (const f of report.findings) {
      if (!byTable[f.table]) byTable[f.table] = [];
      byTable[f.table].push(f);
    }

    for (const [tableName, list] of Object.entries(byTable)) {
      txt += `TABLE: [${tableName}]\n`;
      txt += `-----------------------------------------------------------------\n`;
      txt += `Rule 1: "When Current User is This User" (or Account Owner)\n`;
      txt += `  -> View all fields: YES\n`;
      txt += `  -> Find in searches: YES\n\n`;
      txt += `Rule 2: "Everyone else"\n`;
      txt += `  -> View all fields: NO\n`;
      txt += `  -> DISALLOW viewing following ${list.length} sensitive fields:\n`;
      for (const f of list) {
        txt += `     - [ ] ${f.field} (${f.severity} - ${f.category})\n`;
      }
      txt += `\n\n`;
    }
